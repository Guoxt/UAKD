...
class up_conv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.up = nn.Sequential(
            nn.ConvTranspose2d(in_ch, in_ch, kernel_size=(3,3), stride=(2,2), padding=(1,1), output_padding=(1,1)),
...
class uakd_dual(nn.Module):
    """
    UNet - Basic Implementation
    Paper : https://arxiv.org/abs/1505.04597
    """
    def __init__(self, in_ch=1, out_ch=1):
        super(uakd_dual, self).__init__()

        filters = [8, 8, 16, 8]

        self.Conv1_0 = nn.Sequential(nn.Conv2d(in_ch, filters[0], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[0]),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(filters[0], filters[0], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[0]),
                                     nn.ReLU(inplace=True))
        self.Conv1_1 = nn.Sequential(nn.Conv2d(in_ch, filters[0], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[0]),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(filters[0], filters[0], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[0]),
        ...
        self.Conv2_0 = nn.AdaptiveAvgPool2d((1, 1))
        self.Conv2_1 = nn.Sequential(nn.Conv2d(filters[0], filters[1], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[1]),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(filters[1], filters[1], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[1]),
                                     nn.ReLU(inplace=True))
        
        self.Conv3 = nn.Sequential(nn.Conv2d(filters[2], filters[3], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[3]),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(filters[3], filters[3], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[3]),
                                     nn.ReLU(inplace=True))
        self.Conv4 = nn.Sequential(nn.Conv2d(filters[3], filters[3], kernel_size=1, stride=1, padding=0, bias=True),
                                     nn.BatchNorm2d(filters[3]),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(filters[3], out_ch, kernel_size=1, stride=1, padding=0, bias=True))

        self.active = torch.nn.Sigmoid()

    def forward(self, x):

        e1_0 = self.Conv1_0(x[:,0:1,:,:])
        e1_1 = self.Conv1_0(x[:,1:2,:,:])
        
        e2_0 = self.Conv2_0(e1_0)
        e2_1 = self.Conv2_1(e1_1)        
        
        e2_0 = e2_0.expand(-1, -1, e2_1.size(2), e2_1.size(3))  # 扩展维度到 (batch_size, channels, height, width)
        #print(e2_0.shape, e2_1.shape)
        e2 = torch.cat((e2_0, e2_1), dim=1)
...
class U_Net(nn.Module):
    ...
    def __init__(self, in_ch=1, out_ch=2):
        ...
        self.pre_conv = nn.Sequential(nn.Conv2d(in_ch, 6, kernel_size=3, stride=1, padding=1, bias=True),
                                      nn.BatchNorm2d(6),
                                      nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        fea = self.pre_conv(x)
        #print(torch.max(fea),torch.min(fea))
        e1 = self.Conv1(fea)
...
class Bayesian_U_Net(nn.Module):
    """
    UNet - Basic Implementation
    Paper : https://arxiv.org/abs/1505.04597
    """
    def __init__(self, in_ch=2, out_ch=2):
        super(Bayesian_U_Net, self).__init__()
...
class R2U_Net(nn.Module):
    ...
    """
    def __init__(self, img_ch=1, output_ch=2, t=1):
        super(R2U_Net, self).__init__()

        n1 = 32
        filters = [n1, n1 * 2, n1 * 4, n1 * 8, n1 * 16]

        self.Maxpool = nn.MaxPool3d(kernel_size=(2,2,1), stride=(2,2,1), padding=0)
        self.Maxpool1 = nn.MaxPool3d(kernel_size=(2,2,1), stride=(2,2,1), padding=0)
        self.Maxpool2 = nn.MaxPool3d(kernel_size=(2,2,1), stride=(2,2,1), padding=0)
